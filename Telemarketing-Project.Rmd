...
predictions_glm2_1 <- ifelse(prediction_values_glm2_1 > 0.04, 1, 0) # sensitivity of 0.50400, Pos Pred Value of 0.04880 <- Use this threshold comparison
...
#predictions_glm2_1 <- ifelse(prediction_values_glm2_1 > 0.06, 1, 0) # sensitivity of 0.1040, Pos Pred Value of 0.09091
...
added_predictors_1 <- as.numeric(test_pred1_1) + binaryprediction2_1[,1] + predictions_glm2_1
...
(num_calls_made_ANN1 <- confusion_matrix_model2_ANN1$table[2] + confusion_matrix_model2ANN_1$table[4])
# 26 calls made
...
predicted_correctly_1s <- confusion_matrix_model2_1$table[4]
...
(profit_per_100_ANN1 <- (100 * (num_calls_made_ANN1 / num_potential_calls_ANN1) * success_rate_clusterANN1 * 6) - (100 * (num_calls_made_ANN1 / num_potential_calls_ANN1) * 1))
#1.295337
...
# (number of people we predicted yyes to be 1 in this cluster)
# Add up values predicted to be 1 (from confusion matrix of chosen model for this cluster):
(num_calls_made_1 <- confusion_matrix_glm2_1$table[3] + confusion_matrix_glm2_1$table[4])
...
# 5: Also, calculate what the profit is for this chosen model of this cluster
(profit_1 <- (num_calls_made_1 * success_rate_cluster1 * 6) - (num_calls_made_1 * (num_calls_made_1 / num_potential_calls_1) * 1))
```
...
# (number of people we predicted yyes to be 1 in this cluster)
# Add up values predicted to be 1 (from confusion matrix of chosen model for this cluster):
(num_calls_made_3 <- confusion_matrix_glm2_3$table[3] + confusion_matrix_glm2_3$table[4]) # 446

# 3: Calculate the success rate (number of people we will call 
#    (model predicted yyes to be 1) who actually have a yyes value of 1 
#    divided by the number of people we will call)
(success_rate_cluster3 <- confusion_matrix_glm2_3$table[4] / num_calls_made_3) # 0.3049327

# 4: Calculate the profit per 100 people:
# (100 * (# calls made / # potential calls made) * success rate * 6) -
# (100 * success rate * 1)
(profit_per_100_3 <- (100 * (num_calls_made_3 / num_potential_calls_3) * success_rate_cluster3 * 6) - (100 * success_rate_cluster3 * 1)) # -2.316478

# 5: Also, calculate what the profit is for this chosen model of this cluster
(profit_3 <- (num_calls_made_3 * success_rate_cluster3 * 6) - (num_calls_made_3 * success_rate_cluster3 * 1)) # 680
```
...
we will call 446
success rate is 0.3049
profit per 100 people is -2.316478
profit for this cluster is 680
```
