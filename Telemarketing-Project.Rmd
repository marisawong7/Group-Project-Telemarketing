...
# (100 * (# calls made / # potential calls made) * success rate * 6) -
# (100 * success rate * 1)
(profit_per_100_2 <- (100 * (num_calls_made_2) * success_rate_cluster2 * 6) - (100 * success_rate_cluster2 * 1))
# 5: Also, calculate what the profit is for this chosen model of this cluster
(profit_2 <- (num_calls_made_2 * success_rate_cluster2 * 6) - (num_calls_made_2 * success_rate_cluster2 * 1)) # 680
```

## Conclusion for Cluster 2

For cluster 2, we have chosen a logistic regression model since it has the largest profit per 100 people ($2.072539). In addition, the chosen logistic regression model for cluster  indicates to call 32 people, so it will not be very time consuming to call all these people.
...
(num_potential_calls_total <- num_potential_calls_1 + num_potential_calls_2 + num_potential_calls_5 + nrow(tele_norm_k_means[tele_norm_k_means$cluster == 4,]))

# 2: Get number of people we will call in this cluster 
# (number of people we predicted yyes to be 1 in this cluster)
# Add up values predicted to be 1 (from confusion matrix of chosen model for this cluster):
(num_calls_made_total <- num_calls_made_1 + num_calls_made_2 + num_calls_made_5 + num_yyes_1)

# 3: Calculate the success rate (number of people we will call 
#    (model predicted yyes to be 1) who actually have a yyes value of 1 
#    divided by the number of people we will call)
predicted_correctly_1s_1 <- confusion_matrix_model2_ANN1$table[4]
predicted_correctly_1s_2 <- confusion_matrix_model2_ANN1$table[4]
predicted_correctly_1s_5 <- confusion_matrix_model2_ANN1$table[4]
# use num_yyes_4 for cluster 4 for number of successes

total_successes <- predicted_correctly_1s_1 + predicted_correctly_1s_2 +
                   predicted_correctly_1s_5 + num_yyes_4
